# Agent Relay: план проекта

## Что делаем

`Agent Relay` - отдельный standalone-инструмент для координации
...
```bash
agent-relay heartbeat --task AGT-20260625-014
agent-relay update \
  --task AGT-20260625-014 \
  --status fixing \
...
```bash
agent-relay update --task AGT-20260625-014 --status verifying --next "..."
agent-relay release --task AGT-20260625-014 --reason "iteration finished"
...
git config user.email codex+019eff77@agent-relay.local
```
...
```text
/Users/lev/domain-projects/agent-relay
```
...
remote: git@github.com:LevDomashnih/agent-relay.git
```

GitHub repo пока не создан. `git ls-remote origin` вернул `Repository not
found`. После создания пустого repo `LevDomashnih/agent-relay` нужно:

```bash
cd /Users/lev/domain-projects/agent-relay
...
Проблема: project-local `.agent-relay/state.json` общий только для одного
...
`agent-relay commit`, который временно выставляет env/config только на
один commit.
...
```bash
agent-relay verify-worktree
agent-relay verify-commit
```
...
В future mode можно запускать `agent-relay verify-commit-range` или
...
- Создать пустой GitHub repo `LevDomashnih/agent-relay`.
...
- `.agent-relay/config.json` существует;
- state читается;
- lock file не stale;
...
```bash
agent-relay explain --task AGT-...
agent-relay explain --commit <sha>
```
...
```text
Работай в /Users/lev/domain-projects/agent-relay.

Это отдельный standalone проект Agent Relay: project-local coordination
layer для параллельных AI coding agents. Не привязывай его к Skillspace.

Перед изменениями:
1. cd /Users/lev/domain-projects/agent-relay
2. git status --short
3. pnpm install
4. pnpm run check
5. pnpm run format
...
Не коммить node_modules, dist, .agent-relay state.
```
