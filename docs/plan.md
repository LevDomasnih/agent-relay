# Agent Coordinator: план проекта

## Что делаем

`Agent Coordinator` - отдельный standalone-инструмент для координации
...
```bash
agent-coordinator heartbeat --task AGT-20260625-014
...
```bash
agent-coordinator update --task AGT-20260625-014 --status verifying --next "..."
agent-coordinator release --task AGT-20260625-014 --reason "iteration finished"
...
git config user.email codex+019eff77@agent-coordinator.local
```
...
```text
/Users/lev/domain-projects/agent-coordinator
```
...
packages/cli        # CLI agent-coordinator
...
remote: git@github.com:LevDomashnih/agent-coordinator.git
```

GitHub repo пока не создан. `git ls-remote origin` вернул `Repository not
found`. После создания пустого repo `LevDomashnih/agent-coordinator` нужно:

```bash
cd /Users/lev/domain-projects/agent-coordinator
...
Проблема: project-local `.agent-coordinator/state.json` общий только для одного
...
`agent-coordinator commit`, который временно выставляет env/config только на
один commit.
...
```text
Before editing any file, call agent_coordinator.claim_task.
Before final response, call agent_coordinator.update_task or release_task.
...
```bash
agent-coordinator verify-worktree
agent-coordinator verify-commit
```
...
В future mode можно запускать `agent-coordinator verify-commit-range` или
...
- Создать пустой GitHub repo `LevDomashnih/agent-coordinator`.
...
- `init` создает `.agent-coordinator`;
...
- `.agent-coordinator/config.json` существует;
- state читается;
- lock file не stale;
...
```bash
agent-coordinator explain --task AGT-...
agent-coordinator explain --commit <sha>
```
...
```text
Работай в /Users/lev/domain-projects/agent-coordinator.

Это отдельный standalone проект Agent Coordinator: project-local coordination
layer для параллельных AI coding agents. Не привязывай его к Skillspace.

Перед изменениями:
1. cd /Users/lev/domain-projects/agent-coordinator
2. git status --short
3. pnpm install
4. pnpm run check
5. pnpm run format
...
Не коммить node_modules, dist, .agent-coordinator state.
```
